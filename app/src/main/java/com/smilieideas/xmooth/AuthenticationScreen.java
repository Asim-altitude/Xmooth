...
import com.smilieideas.xmooth.model.AuthCode;
...
public class AuthenticationScreen extends AppCompatActivity {
    ...
    private void createNewQR() {
        ...
        try {
            ...
            try {
                BitMatrix bitMatrix = qrCodeWriter.encode("https://xmoothi.web.app/", BarcodeFormat.QR_CODE, 200, 200);
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                try {
                    ...
                    if (!jsonObject.getString("access_token").equalsIgnoreCase("")){
                        ...
                        sharedPreferences.edit().putString(CloudDB.ACCESS_TOKEN, accessToken).apply();
                        sharedPreferences.edit().putBoolean(CloudDB.IS_LOGGED_IN,true).apply();
